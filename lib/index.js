...
try {
  assert__default["default"](new URL());
}
catch (error) {
...
function resolveUrl(relative, base) {
  /* istanbul ignore next */
  return useNativeURL ? new URL(relative, base) : parseUrl(url__default["default"].resolve(base, relative));
}
...
module.exports = {
  ...
  init() {
    ...
    if (!fs__default["default"].existsSync('./config.json')) {
      fs__default["default"].writeFileSync('./config.json', JSON.stringify({
        appKey: '',
        appSecret: ''
      }, null, 2));
    } else {
      const config = fs__default["default"].readdirSync('./config.json');
